...
const WORKER_URL = (window as any).CHAT_WORKER_URL || 'https://recovery-chat.kidell-powellj.workers.dev';
  ...
  const analyzeThought = async () => {
    ...
    try {
      ...
      if (parsed.distortions && parsed.distortions.length > 0) {
        // Normalize: worker returns reframingQuestions (array), component uses reframeQuestion (string)
        const normalized = parsed.distortions.map((d: any) => ({
          name: d.name,
          explanation: d.explanation,
          reframeQuestion: d.reframeQuestion || (d.reframingQuestions?.[0]) || '',
        }));
